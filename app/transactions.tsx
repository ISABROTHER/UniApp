...
export default function TransactionsScreen() {
  ...
  const fetchUnifiedTransactions = async (userId: string) => {
    ...
    try {
      ...
      // 4. Fetch Rent Payments (Using tenant_id as per types.ts)
  ...
  const handleDownloadReceipt = (ref?: string) => {
    // We are simulating PDF download to keep dependencies clean as instructed
    Alert.alert(
      'Receipt Generated', 
      `The receipt for transaction ${ref || 'unknown'} has been sent to your registered email address.`
    );
  };
                ...
                <View style={styles.transactionBottom}>
                  ...
                  {t.status === 'completed' && (
                    <TouchableOpacity 
                      ...
                      onPress={() => handleDownloadReceipt(t.reference)}
                    >
