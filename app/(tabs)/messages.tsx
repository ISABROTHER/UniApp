...
export default function MessagesScreen() {
  ...
  const fetchThreads = async () => {
    try {
      ...
      const enriched: Thread[] = rawThreads.map((t: any) => {
        const otherId = t.participant_a === user.id ? t.participant_b : t.participant_a;
        const isParticipantA = t.participant_a === user.id;
        const unread = isParticipantA ? (t.unread_count_a ?? 0) : (t.unread_count_b ?? 0);
