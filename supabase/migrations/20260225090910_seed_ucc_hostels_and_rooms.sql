/*
  # Seed UCC-area hostels, rooms, amenities, and images

  1. New Data
    - 12 hostels near UCC campus with realistic Cape Coast addresses
    - Each hostel has 2-4 room types with pricing
    - Each hostel has amenities
    - Each hostel has stock photos from Pexels

  2. Notes
    - Uses a temporary owner from auth.users if available, otherwise creates hostels without FK constraint issues
    - Hostels cover range of price points (GHS 200-1500/semester)
    - Mix of verified/unverified, featured/standard
    - Realistic Cape Coast locations near UCC
*/
...
BEGIN
  ...
  SELECT id INTO dummy_owner FROM auth.users LIMIT 1;
  IF dummy_owner IS NULL THEN
    RETURN;
  END IF;

  INSERT INTO members (id, full_name, email, role)
  VALUES (dummy_owner, 'System Admin', 'admin@ucchousing.com', 'admin')
  ON CONFLICT (id) DO NOTHING;

  -- Hostel 1: Premium near science faculty
  ...
  VALUES (gen_random_uuid(), dummy_owner, 'Cape Royale Hostel', 'Modern premium hostel with fully furnished self-contained rooms, 24/7 security, and high-speed WiFi. Located just 5 minutes walk from the Science Faculty.', 'Amamoma Road, Cape Coast', '5 min walk to Science Faculty', 5.1109, -1.2901, 800, 1500, 60, 12, 4.6, 38, true, true, 'active')
  RETURNING id INTO h1;

  INSERT INTO hostels (id, owner_id, name, description, address, campus_proximity, latitude, longitude, price_range_min, price_range_max, total_rooms, available_rooms, rating, review_count, featured, verified, status)
  VALUES (gen_random_uuid(), dummy_owner, 'Oasis Student Lodge', 'Affordable and clean hostel with spacious rooms and reliable utilities. Perfect for budget-conscious students near OLA area.', 'OLA Road, Cape Coast', '10 min walk to main campus', 5.1135, -1.2862, 350, 600, 80, 25, 4.2, 52, false, true, 'active')
  RETURNING id INTO h2;

  INSERT INTO hostels (id, owner_id, name, description, address, campus_proximity, latitude, longitude, price_range_min, price_range_max, total_rooms, available_rooms, rating, review_count, featured, verified, status)
  VALUES (gen_random_uuid(), dummy_owner, 'Professors Villa Hostel', 'Executive-style accommodation with air-conditioned rooms, study lounges, and gym access. Opposite the main university gate.', 'University Avenue, Cape Coast', '2 min walk to main gate', 5.1098, -1.2915, 1000, 1500, 40, 5, 4.8, 27, true, true, 'active')
  RETURNING id INTO h3;

  INSERT INTO hostels (id, owner_id, name, description, address, campus_proximity, latitude, longitude, price_range_min, price_range_max, total_rooms, available_rooms, rating, review_count, featured, verified, status)
  VALUES (gen_random_uuid(), dummy_owner, 'Apewosika Heights', 'Well-maintained hostel in the Apewosika junction area with generator backup and water storage. Popular among level 200+ students.', 'Apewosika Junction, Cape Coast', '15 min walk to campus', 5.1152, -1.2830, 250, 500, 100, 35, 3.9, 64, false, true, 'active')
  RETURNING id INTO h4;

  INSERT INTO hostels (id, owner_id, name, description, address, campus_proximity, latitude, longitude, price_range_min, price_range_max, total_rooms, available_rooms, rating, review_count, featured, verified, status)
  VALUES (gen_random_uuid(), dummy_owner, 'Amamoma Premier Hostel', 'The closest private hostel to Science Faculty. Featuring study rooms, laundry services, and a rooftop lounge area.', 'Amamoma, Cape Coast', '3 min walk to Science Faculty', 5.1115, -1.2895, 700, 1200, 50, 8, 4.5, 41, true, true, 'active')
  RETURNING id INTO h5;

  INSERT INTO hostels (id, owner_id, name, description, address, campus_proximity, latitude, longitude, price_range_min, price_range_max, total_rooms, available_rooms, rating, review_count, featured, verified, status)
  VALUES (gen_random_uuid(), dummy_owner, 'Students Hub Residences', 'Budget-friendly hostel with clean shared facilities and 24-hour security. Near Cape Coast Technical University junction.', 'CCTU Road, Cape Coast', '20 min walk to UCC', 5.1048, -1.2810, 200, 400, 120, 45, 3.7, 89, false, true, 'active')
  RETURNING id INTO h6;

  INSERT INTO hostels (id, owner_id, name, description, address, campus_proximity, latitude, longitude, price_range_min, price_range_max, total_rooms, available_rooms, rating, review_count, featured, verified, status)
  VALUES (gen_random_uuid(), dummy_owner, 'Sasakawa Suites', 'Named after the Sasakawa area, this modern hostel offers en-suite rooms with kitchenettes and dedicated parking. Ideal for graduate students.', 'Sasakawa, Cape Coast', '8 min drive to campus', 5.1175, -1.2780, 600, 1000, 30, 10, 4.3, 19, false, true, 'active')
  RETURNING id INTO h7;

  INSERT INTO hostels (id, owner_id, name, description, address, campus_proximity, latitude, longitude, price_range_min, price_range_max, total_rooms, available_rooms, rating, review_count, featured, verified, status)
  VALUES (gen_random_uuid(), dummy_owner, 'Kofi Annan Court', 'Premium gated community hostel with 24/7 CCTV, swimming pool access, and dedicated study halls. Walk to Arts Faculty in 5 minutes.', 'Science Junction, Cape Coast', '5 min walk to Arts Faculty', 5.1090, -1.2935, 900, 1400, 45, 7, 4.7, 33, true, true, 'active')
  RETURNING id INTO h8;

  INSERT INTO hostels (id, owner_id, name, description, address, campus_proximity, latitude, longitude, price_range_min, price_range_max, total_rooms, available_rooms, rating, review_count, featured, verified, status)
  VALUES (gen_random_uuid(), dummy_owner, 'Eagle Heights', 'Newly built hostel with modern finishes, elevator access, and backup power. Located behind the new auditorium.', 'Auditorium Road, Cape Coast', '7 min walk to main campus', 5.1125, -1.2880, 500, 900, 70, 20, 4.1, 15, false, false, 'active')
  RETURNING id INTO h9;

  INSERT INTO hostels (id, owner_id, name, description, address, campus_proximity, latitude, longitude, price_range_min, price_range_max, total_rooms, available_rooms, rating, review_count, featured, verified, status)
  VALUES (gen_random_uuid(), dummy_owner, 'Palm Court Hostel', 'Serene hostel surrounded by palm trees with outdoor study areas and BBQ grounds. Perfect for students who enjoy nature.', 'Duakor Road, Cape Coast', '12 min walk to campus', 5.1065, -1.2850, 300, 550, 55, 18, 4.0, 28, false, true, 'active')
  RETURNING id INTO h10;

  INSERT INTO hostels (id, owner_id, name, description, address, campus_proximity, latitude, longitude, price_range_min, price_range_max, total_rooms, available_rooms, rating, review_count, featured, verified, status)
  VALUES (gen_random_uuid(), dummy_owner, 'The Cove Hostel', 'Trendy student hostel with co-working spaces, fiber internet, and an in-house cafeteria. Right at Amamoma roundabout.', 'Amamoma Roundabout, Cape Coast', '4 min walk to campus', 5.1102, -1.2908, 650, 1100, 48, 11, 4.4, 36, true, true, 'active')
  RETURNING id INTO h11;

  INSERT INTO hostels (id, owner_id, name, description, address, campus_proximity, latitude, longitude, price_range_min, price_range_max, total_rooms, available_rooms, rating, review_count, featured, verified, status)
  VALUES (gen_random_uuid(), dummy_owner, 'OLA Junction Hostel', 'Simple and affordable rooms with basic amenities. Great location near the OLA marketplace for daily essentials.', 'OLA Junction, Cape Coast', '10 min walk to campus', 5.1140, -1.2855, 200, 350, 90, 40, 3.5, 71, false, false, 'active')
  RETURNING id INTO h12;

  -- Room types for each hostel
  -- h1: Cape Royale
  INSERT INTO hostel_rooms (hostel_id, room_type, price_per_month, available_count, total_count, description) VALUES
    ...
    (h1, 'Double Room', 600, 3, 15, 'Shared room for two with individual desks and storage');
  ...
  INSERT INTO hostel_rooms (hostel_id, room_type, price_per_month, available_count, total_count, description) VALUES
    ...
    (h9, 'Self-Contained', 900, 6, 25, 'Self-contained room with elevator access'),
  ...
  -- Images for each hostel (using Pexels stock photos)
